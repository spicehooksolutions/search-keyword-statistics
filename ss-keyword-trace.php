...
*/
	function save_keyword( $keyword, $referer ) {
		...
		$referer_info = parse_url( $referer );
                ...
                {
                     if(is_numeric($user))
                        $row=$wpdb->get_var( "SELECT id FROM " . SS_TABLE . " WHERE keywords = '" . ($keyword)."' and user='".($user)."'");
                     else
        ...
        function repeat_count($keyword,$user)
        {
            global $wpdb;
            if(is_numeric($user))
            $repeat_count = $wpdb->get_var( "SELECT repeat_count FROM " . SS_TABLE . " WHERE keywords ='" . ($keyword)."' and user='".($user)."'");
            else
            {
                //echo "SELECT repeat_count FROM " . SS_TABLE . " WHERE keywords ='" . ($keyword)."'";
                $repeat_count = $wpdb->get_var( "SELECT repeat_count FROM " . SS_TABLE . " WHERE keywords ='" . ($keyword)."'");
            }
